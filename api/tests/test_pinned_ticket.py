...
class PinnedTicketTestCase(TestCase):
    ...
    member_of_interest_dict = dict(role="AD", bio="Is not sus")
    ...
    member_not_of_interest_dict = dict(role="AD", bio="Sus!")

    ticket_of_interest_dict = dict(
        title="Ticket Of Interest", description="This is a ticket of interest to us."
    )

    ticket_not_of_interest_dict = dict(
        title="Ticket not of Interest", description="Please ignore this!"
    )
    ...
    def setUp(self):
        ...
        self.user_not_of_interest = User.objects.create(
            **self.user_not_of_interest_dict
        )

        # setup members
        self.member_of_interest = Member.objects.create(
            **self.member_of_interest_dict, owner=self.user_of_interest, team=self.team
        )
        self.member_not_of_interest = Member.objects.create(
            **self.member_not_of_interest_dict,
        ...
        self.ticket_of_interest = Ticket.objects.create(
            **self.ticket_of_interest_dict, team=self.team
        )

        self.ticket_not_of_interest = Ticket.objects.create(
            **self.ticket_not_of_interest_dict, team=self.team
        )
    ...
    def test_ticket_pin(self):
        response = self.client_of_interest.post(
            reverse(
                ...
                kwargs={
                    ...
                    "member_pk": self.member_of_interest.pk,
                },
            ),
            data=dict(ticket=self.ticket_of_interest.pk),
            format="json",
        )
        ...
        self.assertEqual(
            response.data[0].get("ticket").get("id"), self.ticket_of_interest.pk
        )
    ...
    def test_ticket_pin_does_not_cross_users(self):
        ...
        response = self.client_not_of_interest.get(
            reverse(
                ...
                kwargs={
                    "team_pk": self.team.id,
                    "member_pk": self.member_not_of_interest.pk,
                },
            ),
            format="json",
        )
